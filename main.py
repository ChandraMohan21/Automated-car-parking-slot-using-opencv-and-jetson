...
while True:
    ...
    if ret:
        for i in range(len(parking_lot_coords)):
            ...
            # Check if parking_lot is not empty
            if parking_lot.size > 0:
                cv2.imwrite("parking_lots/{}.png".format(i), img=parking_lot)
                ...
                print("Error: Empty parking lot image for parking lot {}".format(i))
        ...
        if len(available_parking_lot) > 0:
            ...
            closest_parking_label= labels[parking_lot_coords.index(closest_parking)] if closest_parking is not None else "N/A"
